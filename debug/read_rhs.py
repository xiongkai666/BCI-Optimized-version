...
import numpy as np
from intanutil_rhs.read_header import read_header
from intanutil_rhs.get_bytes_per_data_block import get_bytes_per_data_block
from intanutil_rhs.read_one_data_block import read_one_data_block
from intanutil_rhs.notch_filter import notch_filter
from intanutil_rhs.data_to_result import data_to_result


def read_data_rhs(filename):
    ...
    with open(filename, 'rb') as fid:
        ...
        if data_present:
            ...
            if header['dc_amplifier_data_saved']:
                data['dc_amplifier_data'] = np.zeros([header['num_amplifier_channels'], num_amplifier_samples],
                                                     dtype=np.uint) * header['dc_amplifier_data_saved']
    ...
    if (data_present):
        ...
        data['compliance_limit_data'] = np.bitwise_and(data['stim_data_raw'],
                                                       32768) >= 1  # get 2^15 bit, interpret as True or False
        data['charge_recovery_data'] = np.bitwise_and(data['stim_data_raw'],
                                                      16384) >= 1  # get 2^14 bit, interpret as True or False
        data['amp_settle_data'] = np.bitwise_and(data['stim_data_raw'],
                                                 8192) >= 1  # get 2^13 bit, interpret as True or False
        data['stim_polarity'] = 1 - (2 * (np.bitwise_and(data['stim_data_raw'],
                                                         256) >> 8))  # get 2^8 bit, interpret as +1 for 0_bit or -1 for 1_bit

        curr_amp = np.bitwise_and(data['stim_data_raw'],
                                  255)  # get least-significant 8 bits corresponding to the current amplitude
        ...
        data['amplifier_data'] = np.multiply(0.195,
                                             (
                                                     data['amplifier_data'].astype(
                                                         np.int32) - 32768))  # units = microvolts
        ...
        data['board_adc_data'] = np.multiply(0.0003125,
                                             (data['board_adc_data'].astype(np.int32) - 32768))  # units = volts
        data['board_dac_data'] = np.multiply(0.0003125,
                                             (data['board_dac_data'].astype(np.int32) - 32768))  # units = volts
