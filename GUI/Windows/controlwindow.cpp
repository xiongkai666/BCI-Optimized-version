...

{
    ...
    */
    ...
    QTabWidget *moduleTabWidget = new QTabWidget(this);

    QWidget *startTab = new QWidget(this);
    startTab->setLayout(mainLayout);
    moduleTabWidget->addTab(startTab, tr("Start"));

    QWidget* analysisTab = new QWidget(this);
    moduleTabWidget->addTab(analysisTab, tr("Analysis"));

    QVBoxLayout *pyLayout = new QVBoxLayout(analysisTab);
    QPushButton *runMainPyButton = new QPushButton(tr("Run Spike Sorting"), this);
    pyLayout->addWidget(runMainPyButton, Qt::AlignCenter, Qt::AlignCenter);
    runMainPyButton->setFixedSize(200,100);
    connect(runMainPyButton, &QPushButton::clicked, this, &ControlWindow::runMainScript);

    analysisTab->setLayout(pyLayout);

    QWidget *reportTab = new QWidget(this);
    moduleTabWidget->addTab(reportTab, tr("Report"));

    QWidget *shareTab = new QWidget(this);
    moduleTabWidget->addTab(shareTab, tr("Share"));

    QWidget *settingsTab = new QWidget(this);
    moduleTabWidget->addTab(settingsTab, tr("Settings"));

    QString styleSheet = "\
        QTabWidget#moduleTabWidget::tab {\
            font-size:28px;\
            color: black;\
            width: 200px; \
            height: 60px;\
        }";
    moduleTabWidget->setStyleSheet(styleSheet);

    setCentralWidget(moduleTabWidget);
...
void ControlWindow::runMainScript() {
    QString scriptPath = "C:/Users/xiongkai/Desktop/BCI-System/X-03/debug/main.py";
    ...
    if (!pythonProcess->waitForStarted()) {
        qDebug() << "Failed to start the process: " << pythonProcess->errorString();
        return;
    }

    if (!pythonProcess->waitForFinished()) {
        qDebug() << "Failed to finish the process: " << pythonProcess->errorString();
        return;
    }

    qDebug() << "Script execution completed successfully.";
