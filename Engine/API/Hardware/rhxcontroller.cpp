...
//   (1) an Pionway XEM6010 USB2/FPGA interface board running the Intan Rhythm interface Verilog code
//       (e.g., a 256-channel Intan RHD2000 USB Interface Board with 256-channel capacity)
//   (2) an Pionway XEM6310 USB3/FPGA interface board running the Intan Rhythm USB3 interface Verilog code
//       (e.g., an Intan Recording Controller with 512-channel or 1024-channel capacity)
//   (3) an Pionway XEM6010 USB2/FPGA interface board running the Intan RhythmStim interface Verilog code
...
// List all available Pionway devices currently connected to this computer.
// Return a vector of strings containing each device's serial number.
vector<string> RHXController::listAvailableDeviceSerials()
{
    int nDevices;
    CPionway *tempDev = new CPionway;
    tempDev->GetDeviceCount(&nDevices);
...
// Find the first Pionway board attached to a USB port and open it.
// Returns 1 if successful, -1 if Pionway cannot be loaded, and -2 if board can't be found.
...
bool RHXController::uploadFPGABitfile(const string& filename)
{
    ...
    UINT32 boardId, boardVersion;
    dev->UpdateWireOuts();
    dev->GetWireOutValue(WireOutBoardId,&boardId);
    dev->GetWireOutValue(WireOutBoardVersion,&boardVersion);
...
bool RHXController::isRunning()
{
    ...
    dev->UpdateWireOuts();
    UINT32 value;
    dev->GetWireOutValue(WireOutSpiRunning,&value);

    // update number of words in FIFO while we're at it
    if (type == ControllerRecordUSB3) {
         dev->GetWireOutValue(WireOutNumWords_USB3,&NumWordsInFifo_USB3);
         lastNumWordsInFifo = NumWordsInFifo_USB3;
    } else {
        dev->GetWireOutValue(WireOutNumWordsMsb_USB2,&NumWordsInFifoMsb_USB2);
        dev->GetWireOutValue(WireOutNumWordsLsb_USB2,&NumWordsInFifoLsb_USB2);
        lastNumWordsInFifo = (NumWordsInFifoMsb_USB2 << 16) + NumWordsInFifoLsb_USB2;
    }
...
{
    long ReadFromBlockPipeOutCount;
    long ReadFromPipeOutCount;
    lock_guard<mutex> lockOk(okMutex);
    ...
    if (type == ControllerRecordUSB3 || is7310) {
        ...
        while (numWordsInFifo() >= usbBufferSize / BytesPerWord) {
            dev->ReadFromBlockPipeOut(PipeOutData, USB3BlockSize, usbBufferSize, usbBuffer, &ReadFromBlockPipeOutCount);
        }
        while (numWordsInFifo() > 0) {
            dev->ReadFromBlockPipeOut(PipeOutData, USB3BlockSize,
                                      ...
                                      usbBuffer, &ReadFromBlockPipeOutCount);
        }
        ...
        while (numWordsInFifo() >= usbBufferSize / BytesPerWord) {
            dev->ReadFromPipeOut(PipeOutData, usbBufferSize, usbBuffer, &ReadFromPipeOutCount);
        }
        while (numWordsInFifo() > 0) {
            dev->ReadFromPipeOut(PipeOutData, BytesPerWord * numWordsInFifo(), usbBuffer, &ReadFromPipeOutCount);
        }
    }
}
...
void RHXController::resetFpga()
{
    lock_guard<mutex> lockOk(okMutex);
    //dev->ResetFPGA();
    dev->ActivateTriggerIn(epAddr_trigIn40, 31);
}
...
//    readTimer.start();
    ...
    if (type == ControllerRecordUSB3 || is7310) {
        dev->ReadFromBlockPipeOut(PipeOutData, USB3BlockSize, BytesPerWord * numWordsToRead, buffer, &result);
    } else {
        dev->ReadFromPipeOut(PipeOutData, BytesPerWord * numWordsToRead, buffer, &result);
    }
...
// Set the eight red LEDs on the Pionway XEM6x10 board according to integer array.
...
void RHXController::uploadCommandList(const vector<unsigned int> &commandList, AuxCmdSlot auxCommandSlot, int bank)
{
    ...
    if (type != ControllerStimRecord) {
        ...
        long writeToPipeInCount = 0;
        switch (auxCommandSlot) {
        case AuxCmd1:
            dev->ActivateTriggerIn(TrigInRamAddrReset_S_USB2, 0);
            dev->WriteToPipeIn(PipeInAuxCmd1Msw_S_USB2, pipeInWidthBytes * (int)pipeInCommandList.size(), commandBufferMsw, &writeToPipeInCount);
            dev->ActivateTriggerIn(TrigInRamAddrReset_S_USB2, 0);
            dev->WriteToPipeIn(PipeInAuxCmd1Lsw_S_USB2, pipeInWidthBytes * (int)pipeInCommandList.size(), commandBufferLsw, &writeToPipeInCount);
            break;
        case AuxCmd2:
            dev->ActivateTriggerIn(TrigInRamAddrReset_S_USB2, 0);
            dev->WriteToPipeIn(PipeInAuxCmd2Msw_S_USB2, pipeInWidthBytes * (int)pipeInCommandList.size(), commandBufferMsw, &writeToPipeInCount);
            dev->ActivateTriggerIn(TrigInRamAddrReset_S_USB2, 0);
            dev->WriteToPipeIn(PipeInAuxCmd2Lsw_S_USB2, pipeInWidthBytes * (int)pipeInCommandList.size(), commandBufferLsw, &writeToPipeInCount);
            break;
        case AuxCmd3:
            dev->ActivateTriggerIn(TrigInRamAddrReset_S_USB2, 0);
            dev->WriteToPipeIn(PipeInAuxCmd3Msw_S_USB2, pipeInWidthBytes * (int)pipeInCommandList.size(), commandBufferMsw, &writeToPipeInCount);
            dev->ActivateTriggerIn(TrigInRamAddrReset_S_USB2, 0);
            dev->WriteToPipeIn(PipeInAuxCmd3Lsw_S_USB2, pipeInWidthBytes * (int)pipeInCommandList.size(), commandBufferLsw, &writeToPipeInCount);
            break;
        case AuxCmd4:
            dev->ActivateTriggerIn(TrigInRamAddrReset_S_USB2, 0);
            dev->WriteToPipeIn(PipeInAuxCmd4Msw_S_USB2, pipeInWidthBytes * (int)pipeInCommandList.size(), commandBufferMsw, &writeToPipeInCount);
            dev->ActivateTriggerIn(TrigInRamAddrReset_S_USB2, 0);
            dev->WriteToPipeIn(PipeInAuxCmd4Lsw_S_USB2, pipeInWidthBytes * (int)pipeInCommandList.size(), commandBufferLsw, &writeToPipeInCount);
            break;
        default:
            cerr << "Error in RHXController::uploadCommandList: auxCommandSlot out of range.\n";
            break;
...
{
    dev_->ActivateTriggerIn(endPointWireInResetRun(), 0x31);
}

// Return 4-bit "board mode" input.
int RHXController::getBoardMode(CPionway* dev_)
{
    dev_->UpdateWireOuts();
    unsigned int boardMode;
    dev_->GetWireOutValue(endPointWireOutBoardMode(),&boardMode);
    return boardMode;
}

// Return number of SPI ports and if I/O expander board is present.
int RHXController::getNumSPIPorts(CPionway* dev_, bool isUSB3, bool& expanderBoardDetected, bool isRHS7310)
{
    ...
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    expanderBoardDetected = (digitalIn & 0x04) != 0;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 2);  // Load digital in shift registers on falling edge of serial_LOAD
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    spiPortPresent[7] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    spiPortPresent[6] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    spiPortPresent[5] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    spiPortPresent[4] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    spiPortPresent[3] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    spiPortPresent[2] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    spiPortPresent[1] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    spiPortPresent[0] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    digOutVoltageLevel = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    userId[2] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    userId[1] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    userId[0] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    serialId[3] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    serialId[2] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    serialId[1] = digitalIn & 0x01;

    pulseWireIn(dev_, WireInSerialDigitalInCntl, 1);
    dev_->GetWireOutValue(WireOutSerialDigitalIn,&digitalIn);
    serialId[0] = digitalIn & 0x01;
...
bool RHXController::isDcmProgDone() const
{
    dev->UpdateWireOuts();
    unsigned int value ;
    dev->GetWireOutValue(WireOutDataClkLocked,&value);
...
void RHXController::pulseWireIn(CPionway* dev_, int wireIn, unsigned int value)
{
